    ...
    <div id="admin-shell" className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
      ...
      <main className="flex-1 p-4 sm:p-8 lg:p-10 pb-24 md:pb-10 overflow-y-auto">
        ...
        {activeTab === 'siswa' && (() => {
          ...
          const currentPage = Math.min(Math.max(1, studentPage), totalPages);
          ...
          return (
            <div className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200/50 shadow-sm">
                <div className="flex-1 flex flex-col sm:flex-row sm:items-center gap-3">
                  <p className="text-xs font-bold text-slate-500 shrink-0">
                    Siswa: {filteredStudents.length > 0 ? startIndex + 1 : 0}-{endIndex} dari {filteredStudents.length} (20/hlm)
                  </p>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        setSelectedAdminStudentClassFilter(e.target.value);
                        setStudentPage(1);
                      }}
                    >
                      <option value="">-- Semua Kelas --</option>
              ...
              {filteredStudents.length === 0 ? (
                ...
                <div className="bg-white rounded-2xl border border-slate-200/60 overflow-hidden shadow-xs">
                  ...
                  {/* Pagination Footer */}
                  {totalPages > 1 && (
                    <div className="bg-slate-50 px-6 py-3.5 border-t border-slate-200/80 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-600 font-medium">
                      <div>
                        Menampilkan <span className="font-bold text-slate-900">{startIndex + 1}</span> - <span className="font-bold text-slate-900">{endIndex}</span> dari <span className="font-bold text-slate-900">{filteredStudents.length}</span> siswa
                      </div>
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white text-slate-700 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed font-semibold text-xs transition cursor-pointer"
                        ...
                        <button
                          ...
                          disabled={currentPage === 1}
                          className="px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white text-slate-700 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed font-semibold text-xs flex items-center gap-1 transition cursor-pointer"
                        >
